...
def bos_up(df, look=30, excl=1):
    ...
    return float(df['close'].iloc[-1])>hh
def bos_dn(df, look=30, excl=1):
    ll=df['low'][:-excl].tail(look).min()
    return float(df['close'].iloc[-1])<ll
...
def market_note():
    ...
    btc_pct=float(tkr["priceChangePercent"]) if tkr and "priceChangePercent" in tkr else None
...
def analyze(sym):
    ...
    trend_up=e20>e50
    m,ms,_=macd(c); macd_up=m.iloc[-1]>ms.iloc[-1]
    adx_val=float(adx(pd.DataFrame({'high':h,'low':l,'close':c}),14).iloc[-1])  # bilgi amaçlı

    v_ok, v = volume_spike(df, n=10, r=1.15)
    if not v_ok: return None,"novol"

    last_down=float(c.iloc[-1])<float(c.iloc[-2])
    side=None
    if trend_up and rr>49: side="BUY"
    elif (not trend_up) and rr<51 and last_down: side="SELL"
    ...
    line=(f"{sym} | 1H | Trend:{'↑' if trend_up else '↓'} | RSI:{rr:.1f} | "
