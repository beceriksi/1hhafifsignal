...
def analyze(sym, interval):
    ...
    if float(df["v"].iloc[-1])<200_000: return None
    c,h,l=df['c'],df['high'],df['low']
    if not gap_ok(c,0.08): return None
    ...
    fr=funding(sym)
    frtxt=f" | Funding:{fr:+.3f}" if fr and abs(fr)>0.01 else ""
    return f"- {sym} | {interval.upper()} | {side} | RSI:{rr:.1f} | ADX:{adx_val:.0f} | Hacim x{v['ratio']:.2f}{frtxt}"
...
def main():
    ...
    if signals:
        msg=f"⚡ *Toplu Sinyaller (1H • 4H • 1D)*\n⏱ {ts()}\n{note}\n\n"+"\n".join(signals[:50])
        telegram(msg)
    else:
        print("ℹ️ Bugün sinyal yok (sessiz geçildi).")
